...
// Pre-loaded punch list floor sections
const PUNCH_FLOORS = [
  "Basement Punch",
  "Ground Floor Punch",
  "1st Floor Punch",
...
const makeDefaultRows = (): PunchRow[] =>
  PUNCH_FLOORS.map(desc => ({ description: desc, qty: 0, unit: "hr", unitCost: 0, laborHrs: 1 }));
...
export default function PunchListTakeoffForm({
  projectId, defaultLaborRate, sectionBreakdown, onCommit, onClose,
}: Props) {
  const [rows,           setRows]           = useState<PunchRow[]>(makeDefaultRows());
  const [laborFactor,    setLaborFactor]    = useState(0);
  const [materialFactor, setMaterialFactor] = useState(0);

  const updateRow = (idx: number, field: keyof PunchRow, value: string | number) =>
    setRows(prev => prev.map((r, i) => i === idx ? { ...r, [field]: value } : r));

  const addRow    = () => setRows(prev => [...prev, { description: "", qty: 0, unit: "hr", unitCost: 0, laborHrs: 1 }]);
  const removeRow = (idx: number) => setRows(prev => prev.filter((_, i) => i !== idx));

  const handleTakeoff = () => {
    const valid = rows.filter(r => r.description.trim() && r.qty > 0);
    if (valid.length === 0) return;
    const category = `Punch List | ${sectionBreakdown.section} | ${sectionBreakdown.breakdown}`;
  ...
  return (
    <TakeoffShell
      title="Punch List Takeoff"
      subtitle={validCount > 0 ? `${validCount} floor${validCount !== 1 ? "s" : ""} with hours` : "Enter hours per floor"}
    ...
    >
      ...
      {/* Hint */}
      <div style={{ padding: "6px 18px", background: "#fff8ed", borderBottom: "1px solid #fde68a", fontSize: 11, color: "#92400e", flexShrink: 0 }}>
        Enter hours for each floor that needs punch work. Leave Qty at 0 to skip a floor.
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 70px 60px 100px 100px 32px", gap: 8, padding: "7px 18px", background: "#dce8f0", borderBottom: "1px solid var(--border)", flexShrink: 0 }}>
        {["Floor / Area", "Qty", "Unit", "Unit Cost", "Labor Hrs ea", ""].map(h => (
      ...
      <div style={{ overflow: "auto", flex: 1, padding: "10px 18px" }}>
        {rows.map((row, idx) => {
          const isPunchFloor = PUNCH_FLOORS.includes(row.description);
          return (
            <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 70px 60px 100px 100px 32px", gap: 8, marginBottom: 7, alignItems: "center" }}>
              <input
                ...
                placeholder="Floor / Area description…"
                style={{
                  ...formInp,
                  fontWeight: isPunchFloor ? 600 : 400,
                  color: isPunchFloor ? "#1a3a6a" : "var(--text-primary)",
                }}
              />
              ...
              <button onClick={() => removeRow(idx)}
                style={{ background: "none", border: "1px solid var(--border-strong)", borderRadius: "var(--r-sm)", color: "var(--red)", cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center", height: 30 }}>
                <Trash2 size={13} />
              </button>
            </div>
          );
